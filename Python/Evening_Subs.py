...
if __name__ == '__main__':
    ...
    f = open(home_dir + "/git/HRPC-YouTube-Scheduler/Service_Details/evening_service_id.txt", "r")
    temp1 = f.readline()
    VIDEO_ID = temp1.strip()
    f.close()

    LANGUAGE = 'en-GB'  # English (United Kingdom)
    f = open(home_dir + "/git/HRPC-YouTube-Scheduler/Service_Details/evening_service_title.txt", "r")
    temp2 = f.readline()
    NAME = temp2.strip()
    f.close()
    ...
    try:
        ...
        msg = f"Evening subtitles successfully uploaded to YouTube for {NAME}"
        print(msg)
        send_notification(msg)
        sys.exit(0)
    except Exception as e:
        err = f"Evening subtitles FAILED uploaded to YouTube for {NAME}"
